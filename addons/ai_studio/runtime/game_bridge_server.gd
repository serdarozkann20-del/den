...
## AI Studio game bridge - runs inside the game while it is played from the editor.
##
## Adapted from godot-mcp's mcp_interaction_server.gd
## (https://github.com/tugcantopaloglu/godot-mcp), MIT License,
## Copyright (c) Tugcan Topaloglu and Solomon Elias. See LICENSE.godot-mcp.txt
## next to this file for the full license text.
##
## Changes for AI Studio: it only starts in a game launched by the editor
## (`editor_runtime` feature) and removes itself everywhere else, so an
## exported build that still carries the autoload is unaffected; the port and a
## per-editor-session token come from the environment the editor passes to the
## game (AI_STUDIO_BRIDGE_PORT / AI_STUDIO_BRIDGE_TOKEN); requests with a wrong
## token are rejected; print output and push_error/push_warning are captured by
## a Logger and served by the get_logs / get_errors commands.
##
## Protocol: newline-delimited JSON over TCP on 127.0.0.1,
## request {"command", "params", "id", "token"} -> response {..., "id"} or {"error"}.
## No class_name on purpose (it is registered as an autoload).
...
const MAX_CAPTURED: int = 2000
var _port: int = DEFAULT_PORT
var _token: String = ""
var _capture: _BridgeLogCapture = null
const BUSY_TIMEOUT: float = 120.0
var _key_map: Dictionary
var _held_keys: Dictionary = {}

func _ready() -> void:
	# Only a game started from the editor gets a bridge. Exported builds (or a
	# plain `godot --path` run) drop the node immediately.
	if not OS.has_feature("editor_runtime") or OS.get_environment("AI_STUDIO_BRIDGE") != "1":
		set_process(false)
		queue_free()
		return
	# Ensure MCP server keeps processing even when game is paused
	process_mode = Node.PROCESS_MODE_ALWAYS
	_port = int(OS.get_environment("AI_STUDIO_BRIDGE_PORT")) if OS.get_environment("AI_STUDIO_BRIDGE_PORT").is_valid_int() else DEFAULT_PORT
	_token = OS.get_environment("AI_STUDIO_BRIDGE_TOKEN")
	_capture = _BridgeLogCapture.new()
	OS.add_logger(_capture)
	_init_key_map()
	_server = TCPServer.new()
	var err: int = _server.listen(_port, "127.0.0.1")
	if err != OK:
		push_error("AI Studio game bridge: failed to listen on port %d, error: %d" % [_port, err])
		return
	print("AI Studio game bridge: listening on 127.0.0.1:%d" % _port)


## Collects print output and engine/script errors for get_logs / get_errors.
## Loggers are called from any thread, hence the mutex.
class _BridgeLogCapture extends Logger:
	var mutex := Mutex.new()
	var logs: Array = []
	var errors: Array = []
	...
	func _log_message(message: String, error: bool) -> void:
		mutex.lock()
		logs.append({"message": message.strip_edges(false, true), "stderr": error})
		if logs.size() > MAX_CAPTURED:
			logs = logs.slice(logs.size() - MAX_CAPTURED)
		mutex.unlock()

	func _log_error(function: String, file: String, line: int, code: String, rationale: String,
			_editor_notify: bool, error_type: int, script_backtraces: Array[ScriptBacktrace]) -> void:
		# push_error/push_warning report the C++ call site; point at the script instead.
		for bt in script_backtraces:
			if bt != null and bt.get_frame_count() > 0:
				file = bt.get_frame_file(0)
				line = bt.get_frame_line(0)
				function = bt.get_frame_function(0)
				break
		var kind := "error"
		match error_type:
			ERROR_TYPE_WARNING:
			...
			ERROR_TYPE_SHADER:
				kind = "shader_error"
		mutex.lock()
		errors.append({"type": kind, "message": rationale if not rationale.is_empty() else code,
			"code": code, "function": function, "file": file, "line": line})
		if errors.size() > MAX_CAPTURED:
			errors = errors.slice(errors.size() - MAX_CAPTURED)
		mutex.unlock()

	func take_logs() -> Array:
		mutex.lock()
		var out := logs
		logs = []
		mutex.unlock()
...
func _process(_delta: float) -> void:
	...
	if _busy and _busy_since > 0.0:
		...
		if elapsed > BUSY_TIMEOUT:
			push_warning("AI Studio game bridge: _busy flag stuck for %.1fs, force-resetting" % elapsed)
			_busy = false
			_busy_since = 0.0
			_current_id = null
	...
	if _server.is_connection_available():
		...
		if new_client != null:
			...
			_client = new_client
			_buffer = ""
			print("AI Studio game bridge: Client connected")
	...
	if status == StreamPeerTCP.STATUS_ERROR or status == StreamPeerTCP.STATUS_NONE:
		print("AI Studio game bridge: Client disconnected")
		_client = null
		_buffer = ""
		_busy = false
		_busy_since = 0.0
...
func _handle_command(json_str: String) -> void:
	...
	if not _token.is_empty() and String(data.get("token", "")) != _token:
		_send_response_raw({"error": "Rejected: wrong or missing bridge token.", "id": req_id})
		return
	...
	match command:
		...
		"terrain":
			_cmd_terrain(params)
		"get_logs":
			_send_response({"success": true, "logs": _capture.take_logs() if _capture != null else []})
		"get_errors":
			_send_response({"success": true, "errors": _capture.take_errors() if _capture != null else []})
		"ping":
			_send_response({"success": true, "pong": true, "scene": str(get_tree().current_scene.scene_file_path) if get_tree().current_scene != null else ""})
		_:
...
func _cmd_screenshot() -> void:
	...
	if image == null:
		_send_response({"error": "Failed to capture screenshot (no rendered frame: the game may be running headless or minimized)"})
		return
...
func _exit_tree() -> void:
	if _capture != null:
		...
		_capture = null
	if _server == null:
		return  # never started (not an editor run)
	_clear_debug_draw()
	if _websocket != null:
		_websocket.close()
		_websocket = null
	...
	if _server != null:
		_server.stop()
		_server = null
	print("AI Studio game bridge: Stopped")
